...
| `LONG_SIM_PROC_MGMT.md` | **长仿真进程管理与安全终止**（P-G/P-E 长跑经验） | `pkill -f <pattern>` 会匹配执行它的 shell 自身 → 自杀式"卡死"；安全法=`pgrep/pkill -x`（精确进程名）、`pkill -f '[y]syx...'` 方括号技巧、或启动记录 PID 按 PID 杀；长跑用 `setsid` 脱离进程组以活过工具超时；前置保险：`SIM_END`、总线上看门狗（仅总线活动有效）、探针 cap、**启动前查镜像大小**（linker 写错会产生 GB 级镜像并空转） |
