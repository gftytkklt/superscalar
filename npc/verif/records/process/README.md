...
| `B3_STAGE8_PB_FMAX.md` | B3 阶段8（八）：P-B 部件级 f_max 评估（#6）+ r 更新决策点 | 探针平台 `verif/sta/probe/probe_run.sh`（nangate45）：**gpr 14,214μm²/0.490ns/2,041MHz；64bit 加减法器 2,111μm²/1.435ns/697MHz**；全芯片 A1 后 426MHz（关键路径仍=icache FSM）→ 数据通路非瓶颈、提频杠杆在 FSM；**r 决策待拍板**（A 保持 3.5 / B 切 4.26 / C 参数化，推荐 C） |
