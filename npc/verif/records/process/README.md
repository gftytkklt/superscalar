...
| `B3_STAGE8_PH.md` | B3 阶段8（十七）：P-H 教学项执行记录（**进行中：H1 ✅ / H2 待做 / H3 待做**） | **H1 #12 完成**：`verif/perf/trace_locality.py`（`--cacheable-only` 剔除 RTL 非缓存 SRAM/MMIO）对 `/tmp/mb_pe2.trace`（取指握手口径，与 ICACHE_STAT 逐位一致）：**F 1.73M 访问仅 286 行/18KB、gap p50=1/99.5%≤64（极强时间局部性）**；与 retire 口径 `/tmp/mb_pe.trace` 对照：F 1.35M、同线 95.3%、跨行仅 +2.9%/−1.8%，差 380,591 次（+28%）=冲刷/未退休取指；**R 85%=UART LSR 单地址 324,820 次**（剔除后仅 34K：堆 66%+flash 34%、gap p50=2）；**W 1067 行≈68KB ≫ 4KB dcache（空间局部性好但容量不足）**；含"怎么读四张图↔ microbench 各阶段 ↔ 计数器"教学小节 |
