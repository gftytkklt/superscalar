...
module pipeline_equiv (
  ...
  // ---------------- 指令字段 ----------------
  wire [6:0] op = wb_ins[6:0];
  wire [4:0] rd = wb_ins[11:7];
  wire [2:0] f3 = wb_ins[14:12];
  wire [4:0] rs1 = wb_ins[19:15];
  wire [4:0] rs2 = wb_ins[24:20];
  wire [6:0] f7 = wb_ins[31:25];
  ...
  // 立即数
  wire [63:0] imm_i = {{52{wb_ins[31]}}, wb_ins[31:20]};
  wire [63:0] imm_s = {{52{wb_ins[31]}}, wb_ins[31:25], wb_ins[11:7]};
  wire [63:0] imm_b = {{51{wb_ins[31]}}, wb_ins[31], wb_ins[7], wb_ins[30:25], wb_ins[11:8], 1'b0};
  wire [63:0] imm_u = {{32{wb_ins[31]}}, wb_ins[31:12], 12'b0};
  wire [63:0] imm_j = {{43{wb_ins[31]}}, wb_ins[31], wb_ins[19:12], wb_ins[20], wb_ins[30:21], 1'b0};
  ...
  wire [63:0] shamt = {58'd0, wb_ins[25:20]};
  wire [63:0] opimm_res =
      (f3 == 3'b000) ? rv1 + imm_i                                   :
      ...
      (f3 == 3'b111) ? (rv1 & imm_i)                                 :
      (f3 == 3'b001) ? (rv1 << shamt)                                :
      /* 101 */ ((wb_ins[30]) ? asr(rv1, shamt[5:0]) : (rv1 >> shamt));

  wire [63:0] op_res =
      (f3 == 3'b000) ? ((wb_ins[30]) ? rv1 - rv2 : rv1 + rv2) :
      (f3 == 3'b001) ? (rv1 << rv2[5:0])                      :
      (f3 == 3'b010) ? (s1 < s2 ? 64'd1 : 64'd0)              :
      (f3 == 3'b011) ? (rv1 < rv2 ? 64'd1 : 64'd0)            :
      (f3 == 3'b100) ? (rv1 ^ rv2)                            :
      (f3 == 3'b101) ? ((wb_ins[30]) ? asr(rv1, rv2[5:0]) : (rv1 >> rv2[5:0])) :
