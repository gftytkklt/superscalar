...
./cachesim /tmp/mb.trace --i sram=4:blk=32:ways=2 --d sram=4:blk=32:ways=2   # 只评估指定配置
```
...
- **成本**：`cachesim.cpp` 的 `RegionCost()` 初值（按区域 `base/wb/mmio`，来自 npc 实测，未校准）；`hit=2`。
  修改该初值即可校正；块大小不同时按 `block/32` 比例缩放 refill/wb。
