...
static void init_elf() {
  ...
  for (int elf_idx=0;elf_idx<elf_num;elf_idx++){
    FILE* elf_fp = fopen(elf_file[elf_idx],"rb");
    Assert(elf_fp, "Can not open %s", elf_file[elf_idx]);
    char buf[BUF_SIZE] = {'\0'};
